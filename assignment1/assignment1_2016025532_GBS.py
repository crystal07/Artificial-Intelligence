...
def greedy_best_first_search (maze, row, col, start) :
    priority_queue = []
    count = 0
    dest_list = find_destination_point(maze, row, col)
    huristic = get_huristic_value (start, dest_list)
        ...
        if maze[pos[1]][pos[0]] == 4 :
            ...
            for i in direction :
                ...
                if maze[new_pos[1]][new_pos[0]] == 4 :
                    ...
                    huristic = get_huristic_value (new_pos, dest_list)
...
dest, parent, count = greedy_best_first_search(maze, row, col, start)
if dest == [-1, -1] :
